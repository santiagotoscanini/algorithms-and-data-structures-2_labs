...
public class BinaryHeapExtended<T> implements PriorityQueueExtended<T> {

	// Attributes
	private Pair<T, Integer>[] table;
	private Integer limit;
	private Integer priority;
	private Hash<Pair<T, Integer>> hashTable;
	...
	public BinaryHeapExtended(Integer size, Function<Pair<T, Integer>, Integer> f) {
		...
		this.limit = 0;
		this.priority = 0;
		this.hashTable = new ClosedHash<Pair<T, Integer>>(size, f, true);
	}
	...
	private void swap(Integer a, Integer b) {
		...
		this.hashTable.addElement(new Pair<T, Integer>(this.table[a].getV1(), a));
		this.hashTable.addElement(new Pair<T, Integer>(this.table[b].getV1(), b));
	...
	public boolean removeMin() {
		if (this.limit != 0) {
			this.hashTable.deleteElement(new Pair<T, Integer>(this.table[1].getV1(), 1));
	...
	public void insert(T t, Integer priority) {
		...
		this.hashTable.addElement(new Pair<T, Integer>(t, this.limit));
		this.mFloat(limit);
	}
	...
	public boolean changePriority(T elem, Integer newPriority) {
		if (this.hashTable.contains(new Pair<T, Integer>(elem, -1))) {
			int elemPos = this.hashTable.get(new Pair<T, Integer>(elem, 2)).getV2();
			this.table[elemPos].setV2(newPriority);
			this.mFloat(elemPos);

			elemPos = this.hashTable.get(new Pair<T, Integer>(elem, 2)).getV2();
			this.sink(elemPos);
			return true;
		}
		return false;
	...
	public boolean delete(T elem) {
		if (this.hashTable.contains(new Pair<T, Integer>(elem, -1))) {
			int elemPos = this.hashTable.get(new Pair<T, Integer>(elem, 2)).getV2();
			this.limit--;
			this.hashTable.deleteElement(new Pair<T, Integer>(this.table[elemPos].getV1(), elemPos));
